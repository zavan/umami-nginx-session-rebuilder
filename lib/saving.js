  ...
  return sql.begin(async (sql) => {
    ...
    const events = await sql`
      SELECT
        ...
        os.city,
        e.created_at,
        cs.created_at AS correct_session_created_at
    ...
    if (event.correctSessionExists) {
      // Update the session with the event creation date
      // if the event was created before the session.
      if (event.created_at < event.correctSessionCreatedAt) {
        await sql`
          UPDATE session
          SET created_at = ${event.createdAt}
          WHERE session_id = ${correctSessionId}
        `;
      }
    } else {
